...
#[test]
fn test_source_map_es5_transform_for_of_mapping() {
    let source = "for (const value of iterable) { value; }";
    ...
    assert!(
        output.contains("__values(iterable)"),
        "expected for-of transform in output: {output}"
    );
    ...
    let (source_line, source_col) = find_line_col(source, "iterable");
    let (output_line, output_col) = find_line_col(&output, "iterable");
    ...
    let mapping = decoded
        ...
        .unwrap_or_else(|| {
            panic!(
                "expected mapping for iterable. mappings: {mappings} output: {output}"
            )
        });
...
#[test]
fn test_source_map_es5_transform_class_mapping() {
    ...
    let output = printer.get_output().to_string();
    assert!(
        output.contains("var Foo") || output.contains("function Foo"),
        "expected ES5 class downlevel output, got: {output}"
    );
    ...
    let (output_line, output_col) = find_line_col(&output, "Foo");
    ...
    let mapping = decoded
        ...
        .unwrap_or_else(|| {
            panic!(
                "expected mapping for Foo. mappings: {mappings} output: {output}"
            )
        });
