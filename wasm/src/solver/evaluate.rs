...
impl<'a, R: TypeResolver> TypeEvaluator<'a, R> {
    ...
    ///
    /// Application types (`Application(Ref(sym), args)`) are expanded to their instantiated form.
    /// For example, `Reducer<number, AnyAction>` where `Reducer<S, A> = (state: S | undefined, action: A) => S`
    /// is expanded to `(state: number | undefined, action: AnyAction) => number`.
    ///
    /// See `evaluate_application` for the expansion logic.
