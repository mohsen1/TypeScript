...
fn test_parent_mapping() {
    ...
    assert!(binary_extended.parent.is_none(),
...
fn test_parent_mapping_nested() {
    ...
    let a = arena.add_identifier(
        SyntaxKind::Identifier as u16,
        0, 1,
        IdentifierData { escaped_text: "a".to_string(), original_text: None, type_arguments: None },
    );
    let b = arena.add_identifier(
        SyntaxKind::Identifier as u16,
        4, 5,
        IdentifierData { escaped_text: "b".to_string(), original_text: None, type_arguments: None },
    );
    let add = arena.add_binary_expr(
        BINARY_EXPRESSION,
        0, 5,
        BinaryExprData { left: a, operator_token: SyntaxKind::PlusToken as u16, right: b },
    );

    let c = arena.add_identifier(
        SyntaxKind::Identifier as u16,
        9, 10,
        IdentifierData { escaped_text: "c".to_string(), original_text: None, type_arguments: None },
    );
    let multiply = arena.add_binary_expr(
        BINARY_EXPRESSION,
        0, 10,
        BinaryExprData { left: add, operator_token: SyntaxKind::AsteriskToken as u16, right: c },
    );
...
fn test_parent_mapping_function() {
    ...
    let name = arena.add_identifier(
        SyntaxKind::Identifier as u16,
        9, 12,
        IdentifierData { escaped_text: "foo".to_string(), original_text: None, type_arguments: None },
    );

    let literal = arena.add_literal(
        SyntaxKind::NumericLiteral as u16,
        23, 25,
        LiteralData { text: "42".to_string(), raw_text: None, value: Some(42.0) },
    );

    let return_stmt = arena.add_return(
        RETURN_STATEMENT,
        16, 26,
        ReturnData { expression: literal },
    );
    ...
    let block = arena.add_block(
        ...
        14, 28,
        BlockData {
            statements: NodeList { nodes: vec![return_stmt], pos: 16, end: 26, has_trailing_comma: false },
            multi_line: true,
        },
    );
    ...
    let func = arena.add_function(
        ...
        FunctionData {
            ...
            parameters: NodeList { nodes: vec![], pos: 13, end: 14, has_trailing_comma: false },
    ...
    // Verify parent chain
    assert_eq!(arena.get_extended(name).unwrap().parent, func, "Function name should have function as parent");
    assert_eq!(arena.get_extended(block).unwrap().parent, func, "Function body should have function as parent");
    assert_eq!(arena.get_extended(return_stmt).unwrap().parent, block, "Return statement should have block as parent");
    assert_eq!(arena.get_extended(literal).unwrap().parent, return_stmt, "Literal should have return statement as parent");
    assert!(arena.get_extended(func).unwrap().parent.is_none(), "Function should have no parent");
}
