...
#[test]
fn test_parity_es5_generator_method_this() {
    let source = r#"
class Counter {
    private count: number = 0;

    *countUp(limit: number): Generator<number> {
...
"#;
    ...
    // Type annotations should be erased
    assert!(
        !output.contains(": number") && !output.contains(": Generator") && !output.contains(": string[]"),
        "Type annotations should be erased: {}",
        output
    );
}
...
#[test]
fn test_parity_es5_generator_method_static() {
    let source = r#"
...
class StringUtils {
    static *split(str: string, separator: string): Generator<string> {
...
"#;
    ...
    // Should contain the classes
    assert!(
        output.contains("NumberGenerator") && output.contains("StringUtils"),
        "Output should contain classes: {}",
        output
    );
    // Type annotations should be erased
...
#[test]
fn test_parity_es5_generator_method_params() {
    let source = r#"
class DataStream<T> {
    private data: T[];
    ...
    *map<U>(transform: (item: T) => U): Generator<U> {
        for (const item of this.data) {
...
"#;
    ...
    assert!(
        ...
        "Output should contain DataStream class: {}",
        output
    );
    // Type annotations should be erased
    assert!(
        !output.contains("<T>") && !output.contains("<U>") && !output.contains(": Generator"),
        "Type annotations should be erased: {}",
        output
    );
}
...
#[test]
fn test_parity_es5_generator_method_inheritance() {
    let source = r#"
...
"#;
    ...
    // Type annotations should be erased
    assert!(
        !output.contains("<T>") && !output.contains(": Generator") && !output.contains("abstract"),
        "Type annotations and abstract should be erased: {}",
        output
    );
}
