#!/usr/bin/env node
/**
 * Find extra TS2705 errors in conformance tests.
 */
...
function runWasm(code, fileName, wasm) {
...
async function main() {
  ...
  for (const filePath of testFiles) {
    ...
    if (extraCodes.includes(2705)) {
      const relPath = filePath.replace(CONFIG.conformanceDir + '/', '');
      extraMatches.push(relPath);
      console.log(`\n=== EXTRA TS2705: ${relPath} ===`);
      console.log('Code snippet (first 40 lines):');
      const lines = cleanCode.split('\n');
      console.log(lines.slice(0, 40).map((l, i) => `${i + 1}: ${l}`).join('\n'));
    }

    if (extraMatches.length >= 5) break;
  }

  console.log('\n\n=== SUMMARY ===');
  console.log(`Total files with extra TS2705: ${extraMatches.length}`);
