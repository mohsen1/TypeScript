#!/usr/bin/env node
/**
 * Find TS2355 false positives in conformance tests.
...
async function main() {
  ...
  for (const filePath of testFiles) {
    ...
    if (extraCodes.includes(2355)) {
      const relPath = filePath.replace(CONFIG.conformanceDir + '/', '');
      matchCount += 1;

      if (matches.length < maxSamples) {
        const messages = wasmDiags
          .filter(d => d.code === 2355)
          .map(d => d.message)
          .filter(Boolean);
        console.log(`\n=== FALSE POSITIVE: ${relPath} ===`);
        if (messages.length > 0) {
          console.log('WASM TS2355 errors:');
          for (const message of messages.slice(0, 4)) {
            console.log(`  ${message}`);
          }
        } else {
          console.log('WASM TS2355 errors: (no message available)');
        }
  ...
  console.log('\n\n=== SUMMARY ===');
  console.log(`Total false positive files (within maxTests): ${matchCount}`);
