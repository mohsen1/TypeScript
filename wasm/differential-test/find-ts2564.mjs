#!/usr/bin/env node
/**
 * Find TS2564 coverage in conformance tests.
...
async function main() {
  ...
  for (const filePath of testFiles) {
    ...
    const hasTsc2564 = tscCodes.has(2564);
    const hasWasm2564 = wasmCodes.has(2564);

    // Missing: TSC has 2564 but WASM doesn't
    if (hasTsc2564 && !hasWasm2564) {
      ...
      if (matches.length < maxSamples) {
        const messages = tscDiags
          ...
          .filter(Boolean);
        console.log(`\n=== MISSING TS2564: ${relPath} ===`);
        if (messages.length > 0) {
          console.log('TSC TS2564 errors:');
          for (const message of messages.slice(0, 4)) {
            console.log(`  ${message}`);
          }
        } else {
          console.log('TSC TS2564 errors: (no message available)');
        }
  ...
  console.log('\n\n=== SUMMARY ===');
  console.log(`Total files missing TS2564 (within maxTests): ${matchCount}`);
