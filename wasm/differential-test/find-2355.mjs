#!/usr/bin/env node
/**
 * Find TS2355 errors in conformance tests.
 */
...
async function main() {
  ...
  for (const filePath of testFiles) {
    ...
    if (extraCodes.includes(2355)) {
      ...
      console.log(`\n=== EXTRA TS2355: ${relPath} ===`);
      console.log('Code snippet (first 50 lines):');
      const lines = cleanCode.split('\n');
      console.log(lines.slice(0, 50).map((l, i) => `${i + 1}: ${l}`).join('\n'));
    }

    if (extraMatches.length >= 10) break;
  }

  console.log('\n\n=== SUMMARY ===');
  console.log(`Total files with extra TS2355: ${extraMatches.length}`);
