#!/usr/bin/env node
/**
 * Find TS2403 extra or missing diagnostics in conformance tests.
...
async function main() {
  ...
  for (const filePath of testFiles) {
    ...
    if (reportExtra && extraCodes.includes(2403)) {
      const relPath = filePath.replace(CONFIG.conformanceDir + '/', '');
      const wasmMessages = wasmDiags.filter(d => d.code === 2403);
      console.log(`\n=== EXTRA TS2403: ${relPath} ===`);
      console.log('WASM TS2403 errors:');
      for (const m of wasmMessages.slice(0, 5)) {
        console.log(`  Line ${m.line}: ${m.message}`);
      }
      console.log('\nCode snippet:');
      const snippetSource = isMultiFile ? rawCode : cleanCode;
    ...
    if (reportMissing && missingCodes.includes(2403)) {
      const relPath = filePath.replace(CONFIG.conformanceDir + '/', '');
      const tscMessages = tscDiags.filter(d => d.code === 2403);
      console.log(`\n=== MISSING TS2403: ${relPath} ===`);
      console.log('TSC TS2403 errors:');
  ...
  console.log('\n\n=== SUMMARY ===');
  if (reportExtra) {
    console.log(`Total files with extra TS2403: ${extraMatches.length}`);
    for (const m of extraMatches) {
      console.log(`  ${m.path}: ${m.messages.length} extra TS2403 errors`);
    }
  }
  if (reportMissing) {
    console.log(`Total files with missing TS2403: ${missingMatches.length}`);
    for (const m of missingMatches) {
      console.log(`  ${m.path}: ${m.messages.length} missing TS2403 errors`);
    }
  }
  process.exit(0);
}
