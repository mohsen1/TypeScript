#!/usr/bin/env node
/**
 * Find TS2322 false positives in conformance tests.
...
async function main() {
  ...
  for (const filePath of testFiles) {
    ...
    if (extraCodes.includes(2322)) {
      const relPath = filePath.replace(CONFIG.conformanceDir + '/', '');
      const messages = wasmDiags.filter(d => d.code === 2322).map(d => d.message).filter(Boolean);
      console.log(`\n=== FALSE POSITIVE: ${relPath} ===`);
      if (messages.length > 0) {
        console.log('WASM TS2322 errors:');
        for (const message of messages.slice(0, 4)) {
          console.log(`  ${message}`);
        }
      } else {
        console.log('WASM TS2322 errors: (no message available)');
      }
