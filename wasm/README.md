...
- Tracks: CLI parser still blocks on TS1005 `=>` expected at `checker.ts:15885` (everyType arrow with optional chaining); emitter expanded JSX parity to cover member namespaced attribute expressions and spread+boolean combos; checker added cross-form property/element narrowing and kept computed element access conservative; LSP added cache reuse coverage for hover→signature help/completions after edit across files; solver added TS_UNSOUNDNESS coverage for `noUncheckedIndexedAccess` array index behavior.
