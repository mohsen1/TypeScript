...
- Overall: Added mixed-case exponent missing-sign leading-zeros numeric index regression, template literal infer from `string`/`` `${string}` `` inputs (current behavior yields never), object trifecta null/undefined rejection, mapped type over string keys key-remap omitting `length`, async nested arrow computed-super key arguments-only test, and async mixed-await sequence source-map mapping; main repo is synced to `origin/rust`; no CLI/LSP feature work in flight.
