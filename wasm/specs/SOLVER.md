...
## 🚨 URGENT: Critical Architectural Risks (MUST FIX NOW)

These issues WILL block us from beating TypeScript-Go if not addressed immediately.

### 1. The "String Bloat" Risk (Critical Performance)

**Problem:** `TypeKey` and related structures use `Arc<str>` or `String` instead of interned `Atom` (u32).
...
**Impact:** Destroys cache locality and forces atomic reference counting in hottest loops (type hashing/equality). In a large project with 100k+ types, this causes massive allocator pressure and L2 cache misses.

**Action Required:**
- [ ] **Strict Rule:** No heap-allocated strings inside `TypeKey`
- [ ] Update `PropertyInfo`, `LiteralValue`, `TypeParamInfo` to use `Atom` (u32) or `SymbolId`
- [ ] `TypeKey` must become a "Plain Old Data" (POD) structure
- [ ] Hashing/Comparing becomes pure integer arithmetic

### 2. The "Salsa Gap" (Architectural Drift) - INTEGRATE SALSA

**Problem:** Design doc mandates query-based architecture (`salsa`), but implementation uses manual `RwLock<HashMap>`.

**Risk:** Building an imperative system impossible to migrate to incremental queries later.

**Action Required:**
- [ ] **Add `salsa` crate** to `wasm/Cargo.toml` dependencies
- [ ] Define the **Query Interface** NOW using Salsa macros:
```rust
...
```
- [ ] All solver components accept `&dyn TypeDatabase` rather than concrete structs
- [ ] Replace `RwLock<HashMap>` interner with Salsa's `#[salsa::interned]`
- [ ] This gives us: **automatic memoization, incremental recomputation, cycle detection**
- [ ] Enables future IDE features: edit file → only re-check affected functions

### 3. The Concurrency Bottleneck (Parallel Checking Blocker)

**Problem:** `TypeInterner` uses a global `RwLock`:
```rust
pub struct TypeInterner {
    key_to_id: RwLock<HashMap<TypeKey, TypeId>>, // <--- CONTENTION
}
```

**Impact:** Parallel function checking involves instantiating generics (creating new types). This "Read Only" phase is actually "Write Heavy" for the interner. All Rayon threads will block waiting to intern `Array<T>` or `Promise<U>`.

**Action Required:**
- [ ] **Option A:** Sharded Locking - Use `dashmap` or 64 `RwLock`s based on `hash(key) % 64`
- [ ] **Option B:** Thread-Local Interning - Batch-merge into global interner
- [ ] **Option C:** Lock-free append-only arena

### 4. The "Spaghetti" Code Risk (Maintainability)

**Problem:** Subtyping logic is duplicated between `check_subtype` (returns bool) and `explain_failure` (returns Reason).

**Risk:** When fixing bugs in one path, the other path drifts. Compiler says "Error" but explanation says "No Error".

**Action Required:**
- [ ] Implement the **Tracer Pattern** - one algorithm, pluggable reporting:
```rust
trait SubtypeTracer {
    fn on_mismatch(&mut self, reason: impl FnOnce() -> SubtypeFailureReason) -> bool;
}

// Fast path: NoOpTracer (compiles to nothing, closure never called)
// Slow path: DiagnosticCollector (constructs error details)
```
- [ ] Delete duplicate `explain_*` methods
- [ ] Merge into single generic `check_relation<T: SubtypeTracer>(...)`
