...
Latest run:
- `./wasm/test.sh` ❌ (build errors: duplicate `type_predicate` in `src/solver/operations.rs` and missing `this_type` in `src/solver/infer_tests.rs`; pre-existing on rust)
