...
## Completed
- [x] Structural parser recovery patterns (2026-01-11): Added three more high-impact recovery patterns. 1) Constructor return type recovery - emit TS2380 "Constructor cannot have a return type annotation" for `constructor(): void`, consume type and continue parsing body. 2) Statements in class bodies recovery - detect stray statements (if/while/for/return/try/throw/with/debugger) in class bodies, emit TS1128 and parse statement to consume tokens and maintain sync. 3) Missing fat arrow recovery - detect `(params) { body }` pattern where `=>` is missing, emit TS1005 "'=>' expected" and continue parsing as arrow function. These patterns prevent parser bailout from larger blocks. Build: WASM compiles successfully.
- [x] Additional parser recovery patterns (2026-01-11): Added three more parser recovery patterns. 1) Enum missing comma recovery - emit TS1005 and continue parsing when comma is missing but next token is valid member start (e.g., `enum E { A = 1 B = 2 }`). 2) const/let/var keyword recovery in class bodies - distinguish between `const() {}` (valid method name) vs `const x = 1` (invalid modifier), emit TS1068 and recover by consuming keyword and treating next identifier as property name. 3) Verified accessor type parameter recovery already implemented - `get<T>()` emits TS1094. Tests: 118 thin_parser tests passed (1 pre-existing failure unrelated to changes).
- [x] Parser recovery improvements (2026-01-11): Improved class member and enum recovery patterns to reduce TS1005/TS1068 false positives. 1) Modified `look_ahead_is_accessor()` to check token after get/set - `get: number`, `get() {}` now correctly parsed as property/method (not accessor), only `get foo() {}` parsed as accessor. 2) Added recovery for 'function' keyword in class members (emits TS1068, continues parsing). 3) Added handling for [expr] computed names in enum members (emits TS1164, parses as computed property for recovery). 4) Added GreaterThanToken to `can_token_start_type()` check - `type Box<T = >` now emits TS1110 instead of TS1005. Tests: 118 thin_parser tests passed (1 pre-existing failure unrelated to changes).
