...
## Current Assignment
Add ES5 type guard patterns parity tests

Per GOALS.md Objective 1: ES5 downleveling correctness

Steps:
1. Add 6 ES5 type guard parity tests to `wasm/src/thin_emitter/emitter_parity_tests.rs`:
   - User-defined type guard
   - typeof type guard
   - instanceof type guard
   - in operator guard
   - Assertion function guard
   - Combined type guard patterns
2. Run `./wasm/test.sh emitter_parity` to verify all tests pass
3. Commit with message: `[wasm] emitter: add type guard parity tests`
