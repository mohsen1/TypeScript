...
- Reduce TS2304 false positives: run `node wasm/differential-test/find-ts2304.mjs --max=500 --samples=10` and pick one sample to fix.
- Add a regression in `wasm/src/thin_checker_tests.rs` or `wasm/src/thin_binder_tests.rs`, then fix scope resolution in `wasm/src/thin_binder.rs` / `wasm/src/thin_checker.rs`.
- Run `./wasm/test.sh <new_test_name>` and report the before/after TS2304 sample list.
