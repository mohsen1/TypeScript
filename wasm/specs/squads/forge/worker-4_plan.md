...
## Current Assignment
- [ ] End-to-end validation (Objective 4): add a compile-and-check regression for a non-trivial generic library snippet in `wasm/src/parallel_tests.rs` (or `wasm/src/thin_checker_tests.rs`) and fix the first panic or mismatch in `wasm/src/checker/mod.rs` or `wasm/src/solver/mod.rs`; run `./wasm/test.sh`.
...
- [ ] Record the minimal repro snippet in `wasm/src/parallel_tests.rs` once fixed.
