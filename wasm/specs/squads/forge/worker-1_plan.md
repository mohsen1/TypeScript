...
## Current Assignment
- [ ] Generic inference hardening (Objective 1): handle circular `extends` constraints and usage-based inference in `wasm/src/solver/infer.rs`; add regressions in `wasm/src/solver/infer_tests.rs`; run `./wasm/test.sh`.
...
- [ ] Compare inference results vs `tsc` for self-referential `infer` in conditional types and document any deltas in the test expectations.
