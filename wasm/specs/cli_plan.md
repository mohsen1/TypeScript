...
- For-in/of nodes now set parent pointers so bound-state scope lookup can walk to block locals; added `test_thin_binder_resolves_block_local_from_bound_state_binder_ts_432`.
- Latest bench gap from rerun after for-of parent wiring: `src/compiler/binder.ts:575:33` (TS2693: `Set` only refers to a type, but is being used as a value here).
