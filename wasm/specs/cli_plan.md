...
- Bench helper: suppress speculative type-argument diagnostics so relational `< ( ... )` expressions parse (fix for `checker.ts:8505`).
- Latest parse gap from bench run: `src/compiler/checker.ts:15885` (TS1005: `=>` expected; arrow inside `everyType` with optional chaining).
...
- `./wasm/bench_cli.sh --repo . --tsconfig src/compiler/tsconfig.json --runs 1 --warmup 1` (failed: tsz diagnostics; first parse error now `src/compiler/checker.ts:15885` (TS1005 `=>` expected)).
