...
Loop:
  ...
  9. **CRITICAL: Sync with origin after EVERY task**
     - Run: `git push origin emitter-track`
     - Run: `git fetch origin && git merge origin/rust`
     - Resolve any conflicts if they occur
     - Run: `git push origin emitter-track` (if there were merges)
...
1. **STAY ON YOUR TRACK** - You are on the **emitter-track**. NEVER switch to checker or lsp work. If your track's work is near perfect, PERFECT IT. Add more tests. Clean up todos. Verify architecture compliance. Polish what you have.
...
If emitter work feels complete, that means you have MORE work to do:
